...
bool DallasTemperature::setResolution(const uint8_t* deviceAddress, uint8_t newResolution, bool skipGlobalBitResolutionCalculation) {
    ...
    if (isConnected(scratchPad, deviceAddress)) {
        ...
        if (deviceAddress[0] != DS18S20MODEL) {
            ...
            writeScratchPad(scratchPad, deviceAddress);
...
float DallasTemperature::getTempCByIndex(uint8_t deviceIndex, bool isDS18S20 = false) {
    ...
    return getTempC((uint8_t*)deviceAddress, isDS18S20);
}
...
float DallasTemperature::getTempFByIndex(uint8_t deviceIndex, bool isDS18S20 = false) {
    ...
    return getTempF((uint8_t*)deviceAddress, isDS18S20);
}
...
void* DallasTemperature::operator new(unsigned int size) { // Implicit NSS obj size
    ...
    //!!! CAN'T EXPLICITLY CALL CONSTRUCTOR - workaround by using an init() method
